    ...
    async def botVersion(self, itx: discord.Interaction):
        ...
        for i in range(len(latest_version.split("."))):
            if int(latest_version.split(".")[i]) > int(self.client.version.split(".")[i]):
                await itx.response.send_message(f"Bot is currently running on v{self.client.version} (latest: v{latest_version})\n(started <t:{unix}:D> at <t:{unix}:T>)", #.strftime('%Y-%m-%dT%H:%M:%S.%f%z')
                                                ephemeral=not public)
                return
        else:
            await itx.response.send_message(f"Bot is currently running on v{self.client.version} (latest)\n(started <t:{unix}:D> at <t:{unix}:T>)",#.strftime('%Y-%m-%dT%H:%M:%S.%f')
                                            ephemeral=not public)
