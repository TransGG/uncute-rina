...
class Tags: # TODO: move to own file
    def __init__(self):
    ...
    async def send_pksettag_info(self, tag_name: str, itx:discord.Interaction, client, public, anonymous):
        embed = discord.Embed(
            ...
            description="Please set a server tag via PluralKit. You can do that by doing `pk;s server tag [tag]` in the bot "
                        "commands channel.\n\nA tag is an emoji or word that represents your system to make it easier for "
                        "moderators and users to identify what system you are a part of without needing to use commands or ask you.\n\n"
                        "Example: Apocalypse System uses ☣️ as a system tag but \"Mod\" as a server tag to be easily identifiable as a moderator."
        )
        await self.tag_message(tag_name, itx, client, public, anonymous, embed)
    #endregion Tags
