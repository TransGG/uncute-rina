...
async def handle_reminder_timestamp_parsing(itx: discord.Interaction, reminder_datetime: str) -> (datetime, discord.Interaction):
    # validate format
    ...
    try:
        ...
        raise ValueError(f"Incorrect format given! I could not convert {reminder_datetime} to format {timestamp_format}")
    ...
    async def remindme(self, itx: discord.Interaction, reminder_datetime: str, reminder: str):
        ...
        try:
                ...
                if possible_timestamp_datetime.isdecimal():
                    if int(possible_timestamp_datetime) < mktime(datetime.timetuple(now)):
                        await itx.response.send_message(
                            ...
                            f"Given timestamp: {int(possible_timestamp_datetime)} (<t:{int(possible_timestamp_datetime)}:F>).\n"
                            f"Current time: {mktime(datetime.timetuple(now))} (<t:{mktime(datetime.timetuple(now))}:F>).\n"
                            ""
                        )
            ...
            try:
                ...
                time_passed: timedelta = distance - now # apparently distance-now does not get typed as timedelta :P silly
                if time_passed > timedelta(days=365*3999):
                    raise ValueError(f"I don't think I can remind you `{time_passed.days//365.2425}` years into the future...")
            except ValueError as ex:
                await itx.response.send_message(
                    f"Couldn't make new reminder:\n> {str(ex)}\n\n"
        ...
        cmd_mention = self.client.get_command_mention("reminder reminders")
        await itx.response.send_message(f"Successfully created a reminder for you on <t:{_distance}:F> for \"{reminder}\"!\nUse {cmd_mention} to see your list of reminders",ephemeral=True)
            ...
            if item is None:
                ...
                if len(out_msg) >= 1995:
                    ...
                    for reminder in db_data['reminders']:
                        ...
                        index += 1
                    cmd_mention = self.client.get_command_mention("reminder reminders")
                    out_msg = f"You have {len(db_data['reminders'])} reminders (use {cmd_mention} `item: ` to get more info about a reminder):\n"+'\n'.join(out)[:1996]
